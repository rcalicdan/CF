...
<div class="fixed inset-0 pointer-events-none" style="z-index: 999999 !important;">
    ...
    <div x-show="orders.length > 0" x-transition:enter="transition ease-out duration-300 transform"
        ...
        x-transition:leave-end="opacity-0 scale-75 translate-y-4" class="absolute bottom-6 right-6 pointer-events-auto">
        ...
        <button @click="optimizeRoutes()" :disabled="loading || orders.length === 0"
            ...
            <!-- Wzbogaczone pływające podpowiedzi -->
            <div
            ...
            <!-- Wzbogaczona odznaka z liczbą zleceń -->
    ...
    <div class="lg:hidden absolute bottom-6 left-1/2 transform -translate-x-1/2 pointer-events-auto"
        ...
        <button @click="optimizeRoutes()" :disabled="loading || orders.length === 0"
            ...
            <span x-show="!loading">Optymalizuj trasy</span>
            <span x-show="loading">Optymalizacja...</span>
            <div
                class="bg-gradient-to-r from-red-500 to-pink-600 text-white text-sm font-bold rounded-full h-7 w-7 flex items-center justify-center shadow-lg border border-white/30">
                <span x-text="orders.length"></span>
            </div>
        </button>
    </div>

<div x-show="optimizationResult || orders.length > 0"
    ...
    <div class="grid grid-cols-2 gap-3 mb-5">
        <div class="text-center p-3 bg-slate-50 rounded-lg border border-slate-200">
            ...
            <div class="text-xs text-slate-500 font-medium">Total Stops</div>
        </div>
        <div class="text-center p-3 bg-slate-50 rounded-lg border border-slate-200">
            <div class="text-xl font-bold text-indigo-600" x-text="orders.filter(o => o.isCustom).length"></div>
            <div class="text-xs text-slate-500 font-medium">Custom Stops</div>
        </div>
    </div>
    ...
    <div class="space-y-4">
        <div class="flex flex-wrap gap-3">
            <button @click="toggleManualEdit()"
                ...
                <span x-text="manualEditMode ? 'Exit Edit Mode' : 'Enter Edit Mode'"></span>
            </button>
        ...
        <div x-show="manualEditMode" x-transition:enter="transition ease-out duration-300"
            ...
            <div x-show="manualEditMode" class="bg-orange-50 border-l-4 border-orange-400 p-4">
                <div class="flex items-start">
                    ...
                    <div class="ml-3">
                        <h4 class="text-base font-semibold text-orange-800">Edit Mode Instructions</h4>
                        <div class="mt-2 text-sm text-orange-700">
                            <ul class="list-disc list-inside space-y-1.5">
                                <li><strong>Drag markers</strong> on the map to reposition stops.</li>
                                <li><strong>Click empty areas</strong> on the map to add new custom stops.</li>
                                <li><strong>Right-click a marker</strong> to see more options.</li>
                                <li><strong>Use the route list</strong> below to drag and reorder stops.</li>
                            </ul>
                        </div>
                    </div>
                </div>
        ...
        <div x-show="manualEditMode && orders.length > 0" x-transition:enter="transition ease-out duration-200"
            ...
            <h4 class="text-base font-semibold text-slate-700 mb-3">Current Route Analysis</h4>
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 text-center">
                ...
                <div class="p-2">
                    ...
                    <div class="text-xs text-slate-500 font-medium">Medium Priority</div>
                </div>
                <div class="p-2">
                    <div class="text-lg font-bold text-green-600"
                        x-text="orders.filter(o => o.priority === 'low').length"></div>
                    <div class="text-xs text-slate-500 font-medium">Low Priority</div>
                </div>
                <div class="p-2">
                    <div class="text-lg font-bold text-slate-800"
                        x-text="'zł' + totalOrderValue.toLocaleString('pl-PL')"></div>
                    <div class="text-xs text-slate-500 font-medium">Total Value</div>
                </div>
            </div>
        </div>
    </div>

        ...
        <form wire:submit.prevent="save" class="p-6 space-y-6">
            <div class="space-y-6">
                <div class="flex items-center justify-between border-b border-gray-200 pb-2">
                    <h3 class="text-lg font-semibold text-gray-900">{{ __('Add Carpet to Order') }}</h3>
                    <div class="text-sm text-gray-600">
                        {{ __('Order') }} #{{ $order->id }} - {{ $order->client->full_name }}
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                <x-utils.link-button :href="route('orders.show', $order)" buttonText="{{ __('Cancel') }}" />
                <x-utils.submit-button wire:target="save" buttonText="{{ __('Add Carpet') }}" />
            </div>
        </form>
    </div>
</div>

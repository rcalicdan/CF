        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <form wire:submit.prevent="update" class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <x-forms.field label="{{ __('Name') }}" name="name" required>
                            <x-forms.input name="name" wire:model="name"
                                placeholder="{{ __('Enter price list name') }}" required :icon="'<path stroke-linecap=\'round\' stroke-linejoin=\'round\' stroke-width=\'2\' d=\'M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z\'></path>'" />
                        </x-forms.field>
                ...
                <div class="space-y-6">
                    <div class="flex items-center justify-between">
                        ...
                        @if (count($servicePrices) > 0)
                            <button type="button" wire:click="togglePreview"
                                ...
                                {{ $showPreview ? __('Hide Service Preview') : __('Show Service Preview') }}
                            </button>
                        @endif
                    </div>
                    <!-- Direct Service Addition Form -->
                    <div class="bg-blue-50 p-4 rounded-lg border border-blue-200">
                        <h4 class="text-sm font-medium text-blue-900 mb-3">{{ __('Add Service (Direct Selection)') }}
                        </h4>
                            ...
                            <div>
                                <x-forms.field label="{{ __('Price') }}" name="price">
                                    <x-forms.input type="number" step="0.01" min="0" name="price"
                                        wire:model="price" placeholder="0.0"/>
                                </x-forms.field>
                            </div>
                            <div>
                    ...
                    @if ($showPreview && count($servicePrices) > 0)
                        <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
                            <div class="px-4 py-3 bg-gray-50 border-b border-gray-200">
                                <h4 class="text-sm font-medium text-gray-900">{{ __('Show Service Preview') }}</h4>
                            </div>
                                            ...
                                            <tr class="hover:bg-gray-50">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    @if ($editingServiceIndex === $index)
                                                        <input type="number" wire:model="editingServicePrice"
                                                            step="0.01" min="0"
                                                            class="w-24 px-2 py-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500">
                                                    @else
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    @php
                                                        $difference =
                                                            $servicePrice['price'] - $servicePrice['base_price'];
                                                        $percentage =
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    @if ($editingServiceIndex === $index)
                                                        <div class="flex items-center space-x-2">
                                                            <button type="button"
                                                                wire:click="saveServicePrice({{ $index }})"
                                                            ...
                                                            <button type="button" wire:click="cancelEditServicePrice"
                                                    ...
                                                    @else
                                                        <div class="flex items-center space-x-2">
                                                            <button type="button"
                                                                wire:click="editServicePrice({{ $index }})"
                    ...
                    @endif
                    <!-- Current Services List (Compact View) -->
                    @if (!$showPreview && count($servicePrices) > 0)
                        <div class="bg-white border border-gray-200 rounded-lg overflow-hidden">
                            <div class="px-4 py-3 bg-gray-50 border-b border-gray-200">
                                <h4 class="text-sm font-medium text-gray-900">{{ __('Service Prices') }}</h4>
                            </div>
                            <div class="divide-y divide-gray-200">
                                @foreach ($servicePrices as $index => $servicePrice)
                                    <div class="px-4 py-3">
                                        <div class="flex items-center justify-between">
                                            <div class="flex-1">
                                                <div class="font-medium text-gray-900">{{ $servicePrice['name'] }}
                                                </div>
                                                <div class="text-sm text-gray-500">{{ __('Base Price') }}:
                                                    {{ number_format($servicePrice['base_price'], 2) }} PLN</div>
                                            </div>
                ...
                <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                    <x-utils.link-button href="{{ route('price-lists.index') }}" buttonText="Cancel" />
                    <x-utils.submit-button wire-target="update" buttonText="Update Price List"

<div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 md:p-8" x-data="laundryChartsComponent()"
    ...
    <div class="mb-6 sm:mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div class="flex gap-2 sm:gap-3 w-full sm:w-auto">
                <button @click="$wire.generatePdfReport()"
                    class="flex-1 sm:flex-none bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                ...
                <button @click="$wire.generateCsvReport()"
                    class="flex-1 sm:flex-none bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">
                    <svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="border-b border-gray-200 mb-6 sm:mb-8">
            <nav class="flex space-x-1 bg-gray-50 rounded-lg p-1">
                <button @click="switchTab('weekly')"
                    :class="activeTab === 'weekly' ? 'bg-white text-emerald-600 shadow-sm' :
                ...
                <button @click="switchTab('yearly')"
                    :class="activeTab === 'yearly' ? 'bg-white text-purple-600 shadow-sm' :
                        'text-gray-500 hover:text-gray-700'"
                    class="flex-1 py-2 px-3 sm:py-3 sm:px-4 text-xs sm:text-sm font-semibold rounded-lg transition-all duration-200">
                    <div class="flex items-center justify-center">
                        <svg class="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" fill="none" stroke="currentColor"
        ...
        <div class="space-y-6 sm:space-y-8">
                        ...
                        <div class="flex space-x-1" x-data="{ showTooltip: false, tooltipType: '' }">
                            <div class="relative">
                                <button @click="toggleChartType('line')"
                                    ...
                                    :class="chartType === 'line' ? 'bg-blue-100 text-blue-600' :
                                ...
                                <div x-show="showTooltip && tooltipType === 'line'" x-transition
                            ...
                            <div class="relative">
                                ...
                                <div x-show="showTooltip && tooltipType === 'bar'" x-transition
                                    class="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs rounded py-1 px-2 whitespace-nowrap z-10">
                                    Wykres słupkowy</div>
                            </div>
                            <div class="relative">
                                <button @click="toggleChartType('area')"
                                    ...
                                    @mouseleave="showTooltip = false"
                                    :class="chartType === 'area' ? 'bg-blue-100 text-blue-600' :
                                        'bg-gray-100 text-gray-600'"
                                    class="p-1.5 sm:p-2 rounded-lg transition-colors duration-200 hover:scale-105">
                                    <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor"
                                        viewBox="0 0 24 24">
                                ...
                                <div x-show="showTooltip && tooltipType === 'area'" x-transition
                                    class="absolute -top-10 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs rounded py-1 px-2 whitespace-nowrap z-10">
                                    Wykres powierzchniowy</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div x-show="activeTab === 'monthly'" x-transition
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 sm:mb-6">
                    ...
                    <span
                        class="text-xs font-medium px-2 py-1 sm:px-3 sm:py-1.5 rounded-full bg-indigo-100 text-indigo-800">Bieżący
                        miesiąc</span>
                </div>
                        ...
                        <template x-for="(item, index) in chartData.statusBreakdown" :key="index">
                            <div
                                ...
                                <div class="flex items-center overflow-hidden">
                                    ...
                                    <div class="flex-1">
                                        <p class="text-sm sm:font-medium text-gray-900 truncate" x-text="item.label">
                                        </p>
                                ...
                                <div class="text-right flex-shrink-0 ml-2">
                                    <p class="text-sm sm:font-bold text-gray-900" x-text="`${item.value} dywanów`">
                                    </p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div
                    ...
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span
                            class="text-xs font-medium px-2 py-1 rounded-full bg-orange-100 text-orange-800">m²</span>
                    </div>
    ...
    <div class="space-y-6 sm:space-y-8 mt-6 sm:mt-8 border-t border-gray-200 pt-6 sm:pt-8">
        <div class="pb-2 border-b border-gray-200">
            ...
            <p class="mt-1 text-sm text-gray-500">
                Analiza przychodów, statusów i kierowców w bieżącym miesiącu.
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div
                class="bg-gradient-to-br from-cyan-50 to-sky-50 rounded-2xl p-4 sm:p-6 border border-cyan-200 shadow-lg">
            ...
            <div
                class="bg-gradient-to-br from-rose-50 to-pink-50 rounded-2xl p-4 sm:p-6 border border-rose-200 shadow-lg">
...
@push('scripts')
    ...
    <script>
        function laundryChartsComponent() {
            return {
                ...
                destroyAllCharts() {
                    ['mainChart', 'statusBreakdownChart', 'completionRateChart', 'averageAreaChart', 'revenueChart',
                        'orderStatusChart', 'driverPerformanceChart'
                    ]
                    .forEach(chartName => {
                        if (this[chartName]) {
                            try {
                        ...
                        tooltip: {
                            ...
                            y: {
                                formatter: (value, {
                                    ...
                                    let text = `${value} dywanów`;
                                    if (item.completion_rate) text += `<br/>Ukończone: ${item.completion_rate}%`;
                                    if (item.avg_area) text += `<br/>Śr. pow.: ${item.avg_area} m²`;
                                    return text;
                                }
                            }
                        },
                ...
                createStatusBreakdownChart() {
                    ...
                    if (!element || this.activeTab !== 'monthly') {
                    ...
                    if (seriesData.length === 0) {
                        element.innerHTML =
                            '<div class="h-full flex items-center justify-center text-gray-500">Brak danych</div>';
                        return;
                    }
                ...
                createAverageAreaChart() {
                    ...
                    const config = {
                        ...
                        series: [{
                            name: 'Śr. pow.',
                            data: data.map(item => item.avg_area || 0)
                        }],
                        xaxis: {
                            categories: data.map(item => item.label || ''),
                            labels: {
                ...
                createRevenueChart() {
                    ...
                    const config = {
                        ...
                        yaxis: {
                            labels: {
                                formatter: (value) => `${(value / 1000).toFixed(1)}k zł`
                            }
                        },
                        colors: ['#3B82F6'],
                        stroke: {
    ...
    <script>
        document.addEventListener('livewire:init', function() {
            ...
            Livewire.on('download-pdf-route', (data) => {
                const filename = data[0].filename;
